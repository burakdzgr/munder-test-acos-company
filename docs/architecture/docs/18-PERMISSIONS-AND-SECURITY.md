...
| S3 | No execution path bypasses the gateway: eslint boundary rule (sandbox/adapters importable only from gateway module) + sandbox-manager rejects exec without valid dispatch token | CI lint + negative integration test |
